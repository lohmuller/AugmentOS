...
export class CoreCommunicator extends EventEmitter {
  ...
  async isBluetoothEnabled(): Promise<boolean> {
    try {
      log.core.info('Checking Bluetooth state...');
      await BleManager.start({ showAlert: false });
      const state = await BleManager.checkState();
      log.core.info('Bluetooth state:', state);
      return state === 'on';
    } catch (error) {
      log.core.error('Error checking Bluetooth state:', error);
      return false;
    }
  }
  ...
  async isLocationPermissionGranted(): Promise<boolean> {
    try {
      ...
      log.core.error('Error checking location permission:', error);
      return false;
    }
  }
  ...
  async isLocationServicesEnabled(): Promise<boolean> {
    try {
      if (Platform.OS === 'android') {
        ...
        log.core.info('Location services enabled (native check):', locationServicesEnabled);
      ...
      log.core.error('Error checking if location services are enabled:', error);
      return false;
    }
  }

  async checkConnectivityRequirements(): Promise<{ isReady: boolean, message?: string }> {
    log.core.info('Checking connectivity requirements');
    ...
    log.core.info('Is Bluetooth enabled:', isBtEnabled);
    if (!isBtEnabled) {
      log.core.info('Bluetooth is disabled, showing error');
      return {
        isReady: false,
    ...
    if (Platform.OS === 'android') {
      ...
      log.core.info('Is Location permission granted:', isLocationPermissionGranted);
      if (!isLocationPermissionGranted) {
        log.core.info('Location permission missing, showing error');
        return {
          isReady: false,
      ...
      log.core.info('Are Location services enabled:', isLocationServicesEnabled);
      if (!isLocationServicesEnabled) {
        log.core.info('Location services disabled, showing error');
        return {
          isReady: false,
    ...
    log.core.info('All requirements met');
    return { isReady: true };
  }
  ...
  async initialize() {
    ...
    try {
      ...
      log.core.warn('Failed to initialize BleManager:', error);
    }
  ...
  private initializeMessageEventListener() {
    ...
    log.core.info('Core message event listener initialized');
  }
  ...
  private handleCoreMessage(jsonString: string) {
    if (INTENSE_LOGGING) {
      log.core.info('Received message from core:', jsonString);
    }
    ...
    try {
      ...
      log.core.error('Failed to parse JSON from core message:', e);
      log.core.error(jsonString);
    }
  }
  ...
  private parseDataFromCore(data: any) {
    ...
    try {
      ...
      log.core.error('Error parsing data from Core:', e);
  ...
  private async sendData(dataObj: any) {
    try {
      if (INTENSE_LOGGING) {
        log.core.info('Sending data to Core:', JSON.stringify(dataObj));
      }
      ...
      log.core.error('Failed to send data to Core:', error);
      GlobalEventEmitter.emit('SHOW_BANNER', {
  ...
  public cleanup() {
    ...
    log.core.info('CoreCommunicator cleaned up');
  }
  ...
  async sendToggleAlwaysOnStatusBar(enabled: boolean) {
    log.core.info('sendToggleAlwaysOnStatusBar');
    return await this.sendData({
