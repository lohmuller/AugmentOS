...
export class CoreCommunicator extends EventEmitter {
  ...
  async isBluetoothEnabled(): Promise<boolean> {
    try {
      log.app.info('Checking Bluetooth state...');
      const state = await BleManager.checkState();
      log.app.info('Bluetooth state:', state);
      return state === 'on';
    } catch (error) {
  ...
  async isLocationServicesEnabled(): Promise<boolean> {
    try {
      if (Platform.OS === 'android') {
        ...
        log.app.info('Location services enabled (native check):', locationServicesEnabled);
  ...
  async checkConnectivityRequirements(): Promise<{ isReady: boolean, message?: string }> {
    log.app.info('Checking connectivity requirements');

    // Check Bluetooth
    const isBtEnabled = await this.isBluetoothEnabled();
    log.app.info('Is Bluetooth enabled:', isBtEnabled);
    if (!isBtEnabled) {
      log.app.info('Bluetooth is disabled, showing error');
      return {
        isReady: false,
  ...
  private handleCoreMessage(jsonString: string) {
    if (INTENSE_LOGGING) {
      log.app.info('Received message from core:', jsonString);
    }
  ...
  private async sendData(dataObj: any) {
    try {
      if (INTENSE_LOGGING) {
        log.app.info('Sending data to Core:', JSON.stringify(dataObj));
      }
