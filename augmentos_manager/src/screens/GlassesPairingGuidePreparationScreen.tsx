...
const GlassesPairingGuidePreparationScreen: React.FC<GlassesPairingGuidePreparationScreenProps> = ({
  ...
  const advanceToPairing = async () => {
    if (glassesModelName == null || glassesModelName == "") {
      log.core.error("SOME WEIRD ERROR HERE");
      return;
    }
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        log.core.info("Requesting PHONE_STATE permission...");
        const phoneStateGranted = await requestFeaturePermissions(PermissionFeatures.PHONE_STATE);
        log.core.info("PHONE_STATE permission result:", phoneStateGranted);
      ...
      log.core.info("Requesting microphone permission...");
      ...
      log.core.info("Microphone permission result:", micGranted);
      ...
      log.core.info("Requesting location permission...");

      // This now handles showing alerts for previously denied permissions internally
      const locGranted = await requestFeaturePermissions(PermissionFeatures.LOCATION);

      log.core.info("Location permission result:", locGranted);
      ...
      log.core.error('Error requesting permissions:', error);
      showAlert(
        'Error',
